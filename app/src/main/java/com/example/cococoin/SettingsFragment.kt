...
class SettingsFragment : Fragment(R.layout.fragment_settings) {
    ...
    private lateinit var btnEmailAccountAction: Button  // 綁定信箱的按鈕
    private lateinit var btnUnlinkGoogleAction: Button  // 解除綁定 Google 的按鈕
    private lateinit var btnUnlinkEmailAction: Button   // 解除綁定信箱的按鈕
    ...
    private fun bindAuthStatus(status: AuthLinkStatus) {
        ...
        val providers = buildList {
            if (status.hasEmailProvider) {
                add(if (status.isEmailVerified) "Email" else "Email（待驗證）")
            }
    ...
    private fun showBindEmailDialog() {
        ...
        val etEmail = dialogView.findViewById<EditText>(R.id.etBindEmail)
        val etPassword = dialogView.findViewById<EditText>(R.id.etBindPassword)
        val etPasswordConfirm = dialogView.findViewById<EditText>(R.id.etBindPasswordConfirm)
        val btnSendCode = dialogView.findViewById<Button>(R.id.btnSendEmailCode)
        val tvHint = dialogView.findViewById<TextView>(R.id.tvEmailCodeHint)

        var verificationMailSent = false

        val dialog = AlertDialog.Builder(requireContext())
            .setTitle("綁定信箱")
            .setView(dialogView)
            .setPositiveButton("我已完成驗證", null)
        ...
        btnSendCode.setOnClickListener {
            ...
            val sendVerification: ((OperationResult) -> Unit) -> Unit = { onComplete ->
                if (verificationMailSent) {
                    authLinkManager.sendEmailVerification(onComplete)
                } else {
                    authLinkManager.linkEmailPassword(email, password, onComplete)
                }
            }

            sendVerification { result ->
                btnSendCode.isEnabled = true
                Toast.makeText(
                    requireContext(),
                    result.message ?: if (result.success) "驗證信已寄出" else "驗證信寄送失敗",
                    Toast.LENGTH_LONG
                ).show()
                ...
                if (result.success) {
                    ...
                    btnSendCode.text = "重新寄送驗證信"
                    tvHint.text = "請到信箱開啟 Firebase 驗證信，點擊信中的驗證連結後，再回到 App 按「我已完成驗證」。"
                }
            }
        }
        ...
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                if (!verificationMailSent) {
                    Toast.makeText(requireContext(), "請先寄送驗證信", Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
                ...
                launchWhenViewActive {
                    ...
                    if (status.isEmailVerified) {
                        ...
                        Toast.makeText(requireContext(), "尚未完成信箱驗證，請先點擊信中的驗證連結", Toast.LENGTH_LONG).show()
                    }
                }
            }
        }
    ...
    private fun confirmUnlinkGoogle() {
        confirmUnlinkProvider(
            ...
            action = { onComplete ->
                authLinkManager.unlinkGoogleProvider(onComplete)
            }
        )
    }
    ...
    private fun confirmUnlinkEmail() {
        confirmUnlinkProvider(
            ...
            action = { onComplete ->
                authLinkManager.unlinkEmailProvider(onComplete)
            }
        )
    }
